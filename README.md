...
structured recap, updated Opportunity, and follow-up tasks - powered by Agentforce.
...
The controller is a thin `@AuraEnabled` facade that delegates to focused, composable services:
...
| `MeetingContextService`        | Account / Opportunity / Contact context resolution.                  |
| `MeetingContentService`        | AI subject/description generation for events and tasks.              |
| `MeetingRecapService`          | Recap submit, insights, commit, and persistence.                     |
...
| `MeetingUtils`                 | Shared JSON, date, and string helpers.                               |
...
3. Click **Recap Meeting** on past events to capture notes (typed or by voice) and generate an AI recap, suggested Opportunity updates, and follow-up tasks.
4. Use **Prep for Meeting** on upcoming events to see related account and contact context at a glance.
5. Review completed recaps and the field changes they drove directly from the dashboard.
